...
async function initializeMqtt() {
    ...
    try {
        ...
        const existing = await db.get('SELECT userId FROM integrations WHERE userId = ? AND type = ?', [DEFAULT_USER_ID, 'mqtt']);
        const configJson = JSON.stringify(mqttConfig);
        const nowMs = Date.now();
        if (existing) {
            // Update existing
            await db.run('UPDATE integrations SET enabled = ?, config = ?, updatedAt = ? WHERE userId = ? AND type = ?', [1, configJson, nowMs, DEFAULT_USER_ID, 'mqtt']);
            console.log('[MQTT Init] Updated existing MQTT integration');
        }
        else {
            // Insert new
            await db.run('INSERT INTO integrations (userId, type, enabled, config, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?)', [DEFAULT_USER_ID, 'mqtt', 1, configJson, nowMs, nowMs]);
        ...
        const verify = await db.get('SELECT userId, type, enabled, config FROM integrations WHERE userId = ? AND type = ?', [DEFAULT_USER_ID, 'mqtt']);
        if (verify) {
