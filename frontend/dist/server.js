...
async function handleDevices(_req, res) {
    try {
        const devices = await (0, device_registration_1.getDevicesForUser)('homeassistant');
...
async function handleRegister(req, res) {
    try {
        ...
        // Claim the entry key
        const serial = await (0, device_registration_1.claimEntryKey)(code, userId);
        if (!serial) {
            res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        await (0, device_registration_1.registerDeviceToUser)(userId, serial);
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({
            success: true,
            serial,
