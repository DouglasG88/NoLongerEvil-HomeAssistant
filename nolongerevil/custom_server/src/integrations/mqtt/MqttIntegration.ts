...
export class MqttIntegration extends BaseIntegration {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // CRITICAL FIX: Wait for device data (capabilities) to load BEFORE running discovery.
      ...
      // 1. Discovery (Create Entities) - Restored to original order
      await this.publishDiscoveryMessages();

      // 2. Initial State (Populate Entities)
    ...
    try {
      ...
      for (const serial of currentSerials) {
        if (!this.userDeviceSerials.has(serial)) {
          ...
          // CRITICAL FIX: Wait for data to arrive before running discovery for this new device
          await this.waitForDeviceData(serial);
